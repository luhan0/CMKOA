function [centers, B] = FastmultiCLR(X,c,anchor_rate,opts,Iter,isGraph)
% Input:
...
%         D = L2_distance_1(X{v}', centers{v}');
%         [di, id] = sort(D, 2); % sort each row
%         di = di(:,1:k+1);
%         id = id(:,1:k+1)';
%         B{v} = zeros(n,m);
%         BB =( (di(:,k+1)-di)./(k.*di(:,k+1)-sum(di(:,1:k)')'+eps) )';
%         B{v}(sub2ind([n, m], reshape([[1:n]'*ones(1,k+1)]',[1,n*k+n]), id(:)')) = BB(:);
